...
# -----------------------------------------------------------------------------
# File: proxy/speaker_logging.py
# Purpose: Structured logging helpers for proxy request lifecycle, diagnostics, and tracing context.
# Status: Active reused observability helper.
# Responsibilities:
...
# Sidecar Contracts:
# - Keep log schema stable for downstream parsing.
...
1)  FastAPI/uvicorn  configure_worker_logging()
2) /


- LOG_INFO_SPEAKERS       1
- LOG_WORKER_COUNT       worker  --workers  WEB_CONCURRENCY/UVICORN_WORKERS
- KEEP_ACCESS_LOG         uvicorn.access 0/false
- LOG_SPEAKER_INDEXES     worker  "0,2" WORKER_INDEX
- WORKER_INDEX            worker /uvicorn


 LOG_SPEAKER_INDEXES  WORKER_INDEX
 pid-hash  LOG_WORKER_COUNT
"""
import logging
import os
import sys
