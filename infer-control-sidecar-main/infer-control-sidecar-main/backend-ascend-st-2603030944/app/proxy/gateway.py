...
# ───────────────────────── 业务路由：与后端路径一一映射 ─────────────────────────
FORCE_TOPK_TOPP = want_topk("WINGS_FORCE_CHAT_TOPK_TOPP", "1")  # 默认开启
...
async def health_get(request: Request):
    """
    GET /health — 代理服务健康检查，复用内部状态机（实现在 health.py）。
    HTTP 状态码语义：200=就绪，201=冷启动中，502=冷启动超时，503=曾就绪后退化。
    """
    h = app.state.health
...
async def health_head(request: Request):
    """
    HEAD /health — 轻量级健康探活，无响应体，仅返回状态码与响应头。
    适用于 Kubernetes readinessProbe/livenessProbe httpGet 场景。
    """
    h = app.state.health
    code = map_http_code_from_state(h)
    headers = build_health_headers(h)
...
async def models_proxy(request: Request):
    """
    GET /v1/models — 透传到后端引擎（vllm/sglang 均原生支持此接口）。
    返回当前已加载的模型列表，供客户端发现模型名称。
    """
