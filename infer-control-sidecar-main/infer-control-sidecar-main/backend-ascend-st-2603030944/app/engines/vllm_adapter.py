...
def _build_base_env_commands(params, engine: str, root: str) -> List[str]:
    """构建基础环境设置命令"""
    env_commands = []
    # 优先使用 app/config/ 内的本地脚本，不再依赖外部 wings/ 目录
...
def build_start_script(params: Dict[str, Any]) -> str:
    ...
    - vllm_ascend：   先 source Ascend CANN/ATB 环境变量，再 exec python3 ...
    """
    engine = params.get("engine", "vllm")
    cmd = _build_vllm_cmd_parts(params)

    if engine == "vllm_ascend":
        # start_command.sh 由 wings-infer 生成，但在 vllm-ascend 引擎容器内执行
        # 引擎容器不包含 wings-infer 的 /app/app/config/ 路径
        # 因此必须 inline 使用镜像内标准路径，而非 source wings-infer 容器内的文件
        # vllm-ascend 官方镜像内置 CANN toolkit，路径固定为：
        #   /usr/local/Ascend/ascend-toolkit/set_env.sh
        #   /usr/local/Ascend/nnal/atb/set_env.sh
        logger.info("vllm_ascend: inline Ascend CANN env setup in start_command.sh")
        # NOTE: k3s 嵌套环境中设备选择说明
        # - ASCEND_VISIBLE_DEVICES 需要 Ascend Docker Runtime hook 才有效，k3s 内无效
        # - vllm TP=1 hardcodes local_rank=0 -> torch.npu.set_device(0)，不受 ASCEND_DEVICE_ID 影响
        # - 在验证环境中使用 NPU 0（物理卡 0），不做设备切换
        # - 若需要指定特定 NPU，需 Ascend Device Plugin for k8s 或重构部署方案
        env_block = (
