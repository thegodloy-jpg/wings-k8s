...
def _get_h20_model_hint() -> str:
    """
    无硬件探测模式下，通过环境变量提示 H20 型号。
    可选值：H20-96G / H20-141G
    """
...
def _load_engine_fallback_defaults(engine: str) -> Dict[str, Any]:
    """
    加载引擎级别的 fallback 默认参数（sglang_default.json / mindie_default.json）。

    当 vllm_default.json 中没有当前模型/引擎的 model_deploy_config 时使用。
    对于 vllm/vllm_ascend 引擎，返回空字典（保持原有行为）。
    """
...
def _write_engine_second_line(path: str, engine: str) -> None:
    ...
    该写入仅用于兼容旧链路的日志/状态标记，不应影响主启动流程。
    因此采用“尽力写入”策略：目录或文件缺失时自动创建，写入失败仅告警。
    """
    try:
