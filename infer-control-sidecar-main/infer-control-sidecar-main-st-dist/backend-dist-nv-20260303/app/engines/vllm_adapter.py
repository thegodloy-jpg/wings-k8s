...
def build_start_script(params: Dict[str, Any]) -> str:
    ...
    # NODE_IPS: comma-separated list of all node IPs (e.g. "7.6.52.110,7.6.52.170")
    ...
    if is_distributed and nnodes > 1:
        ...
        if backend == "ray":
            ...
            if node_rank == 0:
                ...
                script_parts.append(f"ray start --head --port=6379 --node-ip-address=$VLLM_HOST_IP {ray_head_resource} --dashboard-host=0.0.0.0\n")
                ...
                if is_ascend:
                    ...
                    node_ips_bash = node_ips  # e.g. "7.6.52.110,7.6.52.170"
                    script_parts.append(f"NODE_IPS_LIST=\"{node_ips_bash}\"")
                    script_parts.append("HEAD_IP=\"\"")
                    script_parts.append("echo \"[worker] Scanning NODE_IPS for Ray head on port 6379...\"")
                    script_parts.append("for attempt in $(seq 1 120); do")
                    script_parts.append("  for ip in $(echo $NODE_IPS_LIST | tr ',' ' '); do")
                    script_parts.append("    if python3 -c \"import socket; s=socket.socket(); s.settimeout(2); s.connect(('$ip',6379)); s.close()\" 2>/dev/null; then")
                    script_parts.append("      HEAD_IP=$ip")
                    script_parts.append("      echo \"[worker] Found Ray head at $HEAD_IP:6379\"")
                    ...
                    script_parts.append("export HCCL_IF_IP=$(python3 -c \"import socket; s=socket.socket(socket.AF_INET,socket.SOCK_DGRAM); s.connect(('$HEAD_IP',6379)); print(s.getsockname()[0]); s.close()\" 2>/dev/null || hostname -i)")
                    script_parts.append("export HCCL_SOCKET_IFNAME=$(awk '$2==\"00000000\"{print $1;exit}' /proc/net/route 2>/dev/null || echo eth0)")
                ...
                script_parts.append(f"exec ray start --address=$HEAD_IP:6379 --node-ip-address=$VLLM_HOST_IP {ray_worker_resource} --block")
        else: # dp_deployment
            if node_rank == 0:
