...
TARGET_FILE="/opt/vllm_env/lib/python3.10/site-packages/ray/_private/worker.py"

# 2. 定义“已修改”的特征代码（从修改后的内容中选一段唯一的行）
MODIFIED_MARK="pass #faulthandler.enable"

# 3. 检查文件中是否已存在特征代码（-q 静默模式，只返回存在/不存在的状态）
if ! grep -q "$MODIFIED_MARK" "$TARGET_FILE"; then
    # 4. 不存在特征代码 → 执行 sed 修改
    sed -i 's/faulthandler.enable/pass #faulthandler.enable/' "$TARGET_FILE"
