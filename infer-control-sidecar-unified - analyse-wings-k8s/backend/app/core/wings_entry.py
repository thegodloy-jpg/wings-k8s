...
def _shell_escape_single_quote(value: str) -> str:
    ...
    在 bash 单引号字符串中，单引号本身无法直接转义。
    标准做法是结束当前单引号 → 插入转义单引号 → 重新开始单引号：
        例如: it's → 'it'"'"'s'
...
def _build_accel_env_line(engine: str) -> str:
    ...
    用户提供的值会先做 JSON 格式校验（防止注入非法内容），
    再对单引号进行 shell 安全转义。

    Args:
        engine: 引擎名称（vllm / vllm_ascend / sglang / mindie）

    Returns:
        str: 形如 ``export WINGS_ENGINE_PATCH_OPTIONS='...'\\n`` 的 shell 语句。
             若引擎不在已知映射中，返回带注释的空行。
    """
    # 用户显式提供的值优先
    user_override = os.getenv("WINGS_ENGINE_PATCH_OPTIONS", "").strip()
    if user_override:
        # 验证输入是合法的 JSON（防止注入非 JSON 的 shell 命令）
        try:
...
def build_launcher_plan(launch_args: LaunchArgs, port_plan: PortPlan) -> LauncherPlan:
    ...
    # ── Accel 加速包环境注入 ──────────────────────────────────────────────
    # 当 ENABLE_ACCEL=true 时，向脚本头部注入 WINGS_ENGINE_PATCH_OPTIONS
    # 环境变量，告诉 wings_engine_patch 包激活哪些引擎补丁。
    # 此环境变量会在引擎进程（exec）中继承。
    accel_preamble = ""
