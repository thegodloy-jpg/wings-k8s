...
from .tags import build_backend_url
...
def _sglang_pid_grace(now: float, h: dict, pid_alive: bool, backend_ok: bool, http_code: int, err_kind: str) -> bool:
    """
    PID 保护窗口（仅读超时 & 近期有成功）：
      - 仅当 pid_alive 且 backend_ok=False 且 http_code==0 且 err_kind=read_timeout
      - 且 now - last_success_ts ≤ SGLANG_PID_GRACE_MS
    """
    if not pid_alive or backend_ok:
        return False
    if not (http_code == 0 and err_kind == "read_timeout"):
        return False
    last_ok = h.get("last_success_ts")
    if last_ok is None:
        return False
    return (now - last_ok) * 1000.0 <= SGLANG_PID_GRACE_MS
...
async def tick_observe_and_advance(h: dict, client: httpx.AsyncClient) -> None:
    ...
    # 3) 刷新观测数据
    h["pid"] = pid
    h["pid_alive"] = pid_alive
    h["backend_ok"] = backend_ok
    h["backend_http_code"] = http_code
    h["backend_http_latency_ms"] = latency_ms
    h["last_observed_ts"] = _now()
    h["last_error_kind"] = err_kind

    # 4) 状态机推进（保留原有语义）
    if pid_alive and backend_ok:
        # 达到就绪：清零失败，标记曾就绪
        h["status"] = 1
        h["ever_ready"] = True
    ...
    # 5) sglang 专用：积分与读超时计数（其他后端零影响）
    if _is_sglang():
        now = _now()

        # 连续读超时计数（只追踪 read_timeout）
        if not backend_ok and http_code == 0 and err_kind == "read_timeout":
            h["consecutive_timeouts"] = int(h.get("consecutive_timeouts", 0)) + 1
        else:
            h["consecutive_timeouts"] = 0 if backend_ok else h.get("consecutive_timeouts", 0)
        ...
        if pid_alive and backend_ok:
            ...
            if _sglang_pid_grace(now, h, pid_alive, backend_ok, http_code, err_kind):
                pass
            else:
                w = _sglang_weight(http_code, err_kind)
                if w > 0.0:
                    h["fail_score"] += w
                    h["accum_fail_ms"] += int(w * min(latency_ms, HEALTH_TIMEOUT_MS))
...
def map_http_code_from_state(h: dict) -> int:
    ...
    # —— 非 sglang：保持原逻辑 ——
    if (h["consecutive_failures"] >= FAIL_THRESHOLD and
        h["consecutive_failures"] * HEALTH_TIMEOUT_MS >= FAIL_GRACE_MS and
        not (h["pid_alive"] and not h["backend_ok"])):
        return 503
    return 200
