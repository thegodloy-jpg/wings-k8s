# Wings Sidecar 迁移方案 v4（Launcher 模式）
...
4. 启动 proxy 进程（18000）。
5. 启动 health 进程（19000，复用 `wings/wings/proxy/health_service.py`）。
6. 由 health 进程内部轮询 `http://127.0.0.1:17000/health`，对外返回阶段性健康状态。
7. 进入前台守护循环，接收 `SIGTERM/SIGINT` 并清理子进程。
